...
- <u>rd_transform</u>: processes data.
...
Note: This argument is treated as a vector, which means that we can add a filter to each any of the specified variables. Also, even if the filter argument is used to apply the branching logic condition, the warning about the presence of a branching logic is still be displayed, so we can ignore it in this case.
